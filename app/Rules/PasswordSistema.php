...
class PasswordSistema implements Rule, DataAwareRule, ValidatorAwareRule
{
    use Conditionable;
    ...
    public static function defaults($callback = null)
    {
        ...
        if (! is_callable($callback) && ! $callback instanceof static) {
            throw new InvalidArgumentException('La devolución de llamada dada debe ser invocable o una instancia de '.static::class);
        }
    ...
    public function passes($attribute, $value)
    {
        ...
        if ($this->mixedCase && ! preg_match('/(\p{Ll}+.*\p{Lu})|(\p{Lu}+.*\p{Ll})/u', $value)) {
            $this->fail('El :attribute debe contener al menos una letra mayúscula y una minúscula.');
        }

        if ($this->letters && ! preg_match('/\pL/u', $value)) {
            $this->fail('El :attribute debe contener al menos una letra.');
        }

//        if ($this->symbols && ! preg_match('/\p{Z}|\p{S}|\p{P}/u', $value)) {
        if ($this->symbols && ! preg_match('/\p{S}|\p{P}/u', $value)) {
            $this->fail('El :attribute debe contener al menos un símbolo.');
        }

        if ($this->numbers && ! preg_match('/\pN/u', $value)) {
            $this->fail('El :attribute debe contener al menos un número.');
        }

        if ($this->spaces &&  preg_match('/\p{Z}/u', $value)) {
            $this->fail('El :attribute no debe contener Espacios en Blancos.');
        }
        ...
        if ($this->uncompromised && ! Container::getInstance()->make(UncompromisedVerifier::class)->verify([
            ...
            return $this->fail(
                'El dado :attribute ha aparecido en una fuga de datos. Por favor elija una diferente :attribute.'
            );
        }
